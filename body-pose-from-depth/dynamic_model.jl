...
@compiled @gen function step(t::Int, pose_and_image::Tuple{BodyPose,Matrix{Float64}}, renderer::BodyPoseDepthRenderer)
...
@compiled @gen function dynamic_generative_model(num_steps::Int, renderer::BodyPoseDepthRenderer)
    init_pose::BodyPose = @addr(body_pose_model(), :init_pose)
    image::Matrix{Float64} = render(renderer, init_pose)
    blurred::Matrix{Float64} = imfilter(image, Kernel.gaussian(1))
    observable::Matrix{Float64} = @addr(noisy_matrix(blurred, 0.1), :init_image)
    hmm_change::MarkovChange = MarkovChange(true, false, false) # TODO ?
